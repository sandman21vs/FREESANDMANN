...
def _extract_article_form_data(form_data):
    ...
    return {
        ...
        "pinned": 1 if form_data.get("pinned") else 0,
...
def create_admin_article(form_data):
    article_data, error = _extract_article_form_data(form_data)
    if error:
        return {
            "ok": False,
            "message": error,
    ...
    if article_data["publish_mode"] == "override":
...
def update_admin_article(article_id, form_data):
    ...
    published = 1 if article_data["publish_mode"] == "override" else 0
